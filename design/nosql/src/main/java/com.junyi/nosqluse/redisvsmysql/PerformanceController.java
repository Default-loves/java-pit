package com.junyi.nosqluse.redisvsmysql;
...
public class PerformanceController {
    ...
    @GetMapping("redis")
    public void redis() {
        Assert.assertTrue(stringRedisTemplate.opsForValue().get(
                "item" + (ThreadLocalRandom.current().nextInt(CommonMistakesApplication.ROWS) + 1)).equals(CommonMistakesApplication.PAYLOAD));
    }
    ...
    /** Advoid to use keys("item*"), it's very slow, equal scan whole table in mysql */
    @GetMapping("redis2")
    public void redis2() {
