package com.junyi.dataandcode.sqlinject;
...
public class SqlInjectController {
    ...
    public void jdbcwrong(@RequestParam("name") String name) {
        //curl -X POST http://localhost:12345/sqlinject/jdbcwrong\?name\=test
        //python sqlmap.py -u  http://localhost:12345/sqlinject/jdbcwrong --data name=test --current-db --flush-session
        //python sqlmap.py -u  http://localhost:12345/sqlinject/jdbcwrong --data name=test --tables -D "test"
        //python sqlmap.py -u  http://localhost:12345/sqlinject/jdbcwrong --data name=test -D "test" -T "userdata" --dump
        log.info("{}", jdbcTemplate.queryForList("SELECT id,name FROM userdata WHERE name LIKE '%" + name + "%'"));
    }

    /** 在 SQL 语句中使用“?”作为参数占位符，然后提供参数值。
     *  use "?" in sql query instead of concat string */
    @PostMapping("jdbcright")
    public void jdbcright(@RequestParam("name") String name) {
        //curl -X POST http://localhost:12345/sqlinject/jdbcright\?name\=test
        log.info("{}", jdbcTemplate.queryForList("SELECT id,name FROM userdata WHERE name LIKE ?", "%" + name + "%"));
    }

    @PostMapping("mybatiswrong")
    public List mybatiswrong(@RequestParam("name") String name) {
        //curl -X POST http://localhost:12345/sqlinject/mybatiswrong\?name\=test
        //python sqlmap.py -u  http://localhost:12345/sqlinject/mybatiswrong --data name=test --current-db --flush-session
        return userDataMapper.findByNameWrong(name);
    }

    @PostMapping("mybatisright")
    public List mybatisright(@RequestParam("name") String name) {
        //curl -X POST http://localhost:12345/sqlinject/mybatisright?name\=test
    ...
    public List mybatiswrong2(@RequestParam("names") String names) {
        //curl -X POST http://localhost:12345/sqlinject/mybatiswrong2\?names\=\'test1\',\'test2\'
        //python sqlmap.py -u  http://localhost:12345/sqlinject/mybatiswrong2 --data names="'test1','test2'"
    ...
    public List mybatisright2(@RequestParam("names") List<String> names) {
        //curl -X POST http://localhost:12345/sqlinject/mybatisright2\?names\=test1,test2
